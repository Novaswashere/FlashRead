...
export interface FontSectionProps {
  ...
  onFontChange: (font: "Inter" | "Open Sans" | "Merriweather" | "JetBrains Mono") => void;
...
export const FontSection: React.FC<FontSectionProps> = ({
  ...
  const fonts: Array<"Inter" | "Open Sans" | "Merriweather" | "JetBrains Mono"> = [
    "Inter",
    "Open Sans",
    "Merriweather",
    "JetBrains Mono"
    ...
    <div className="p-space-md flex flex-col gap-space-md">
          ...
          {fonts.map((f) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg border-2 ${
                currentFont === f ? "border-primary font-bold" : "border-border-subtle hover:bg-surface-container-low"
              }`}
            >
              {f}
            </button>
      ...
      <div className="flex flex-col gap-2 mt-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-label-mono text-primary font-bold">{currentFontSize}px</span>
        </div>
        <Slider
          max="96"
          min="16"
